...
1. **[Security]** **examples/live_verification.yml:47** Internal gateway URLs and GCP project IDs are hardcoded in a file being added to the repository.
   - **Evidence**: `rdu:
        gateway: "https://og.apps.ocp.rdu.eng.ansible.com"
        bearer_token: "{{ lookup('env', 'OPENSHELL_BEARER_TOKEN_RDU') }}"
      sno:
        gateway: "https://ogo-e2e-c95d70b.apps.sno.rdu.eng.ansible.com"
        bearer_token: "{{ lookup('env', 'OPENSHELL_BEARER_TOKEN_SNO') }}"

    ...
    claude_vertex_project: itpc-gcp-ansible-pe-eng-claude
    gemini_vertex_project: itpc-gcp-ansible-pe-eng-claude`
   - **Confidence**: MEDIUM
   - **Points**: 5
